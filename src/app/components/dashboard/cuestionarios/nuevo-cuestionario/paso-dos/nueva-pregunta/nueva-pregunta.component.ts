...
export class NuevaPreguntaComponent implements OnInit {
  ...
  eliminarRespuesta(index:number):void{
    if(this.getRespuestas.length ===2){
      this.toastr.error('La pregunta debe contener al menos dos respuestas', 'Error!');
    }else{
  ...
  agregarPregunta(): void{
    ...
    //Obtenemos el artray de respuestas
    const arrRespuestas =this.nuevaPregunta.get('respuestas')!.value;

    //Creamos un array de respuestas
    const arrayRta: Respuesta[] = [];

    arrRespuestas.forEach((element: any, index: any ) => {
